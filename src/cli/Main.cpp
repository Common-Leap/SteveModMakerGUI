...
cv::Mat CreateCapeRenderLayer(const cv::Mat& cape) {
	...
	auto warp_face = [&](cv::Rect atlas_rect, const std::array<cv::Point2f, 4>& quad, double gain) {
		cv::Mat face = cape(atlas_rect).clone();
		cv::resize(face, face, cv::Size(1000, 1600), 0, 0, cv::INTER_NEAREST);
		for (int y = 0; y < face.rows; ++y) {
			cv::Vec4b* row = face.ptr<cv::Vec4b>(y);
			for (int x = 0; x < face.cols; ++x) {
				for (int channel = 0; channel < 3; ++channel) {
					row[x][channel] = cv::saturate_cast<uint8_t>(row[x][channel] * gain);
				}
			}
		}
